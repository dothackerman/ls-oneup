...
DROP TRIGGER IF EXISTS probes_submitted_update_guard;

CREATE TRIGGER probes_submitted_insert_guard
BEFORE INSERT ON probes
FOR EACH ROW
WHEN NEW.submitted_at IS NOT NULL
BEGIN
  SELECT CASE
    WHEN length(trim(COALESCE(NEW.submission_ciphertext, ''))) = 0
      AND length(trim(COALESCE(NEW.crop_name, ''))) = 0
    THEN RAISE(ABORT, 'submitted probe requires crop_name or submission_ciphertext')
  END;

  SELECT CASE
    WHEN length(trim(COALESCE(NEW.submission_ciphertext, ''))) = 0
      AND (
        NEW.plant_vitality IS NULL
  ...
  SELECT CASE
    WHEN length(trim(COALESCE(NEW.submission_ciphertext, ''))) = 0
      AND (NEW.gps_lat IS NULL OR NEW.gps_lat < -90 OR NEW.gps_lat > 90)
    THEN RAISE(ABORT, 'submitted probe has invalid gps_lat')
  END;

  SELECT CASE
    WHEN length(trim(COALESCE(NEW.submission_ciphertext, ''))) = 0
      AND (NEW.gps_lon IS NULL OR NEW.gps_lon < -180 OR NEW.gps_lon > 180)
    THEN RAISE(ABORT, 'submitted probe has invalid gps_lon')
  END;

  SELECT CASE
    WHEN length(trim(COALESCE(NEW.submission_ciphertext, ''))) = 0
      AND length(trim(COALESCE(NEW.gps_captured_at, ''))) = 0
    THEN RAISE(ABORT, 'submitted probe requires gps_captured_at')
  END;

  SELECT CASE
    WHEN length(trim(COALESCE(NEW.submission_ciphertext, ''))) = 0
      AND length(trim(COALESCE(NEW.image_key, ''))) = 0
